...
import { MultisigContext, MultisigContextParams } from "../../src";
import { ADDRESS_ONE } from "../constants";

describe("Context instances", () => {
  let contextParams: MultisigContextParams;
  const TEST_WALLET =
  ...
  it("Should create an empty context and have default values", () => {
    const context = new MultisigContext();
    expect(context).toBeInstanceOf(MultisigContext);
  ...
  it("Should create a context and have the correct values", () => {
    const context = new MultisigContext(contextParams);
  ...
  it("Should create a context with invalid network and fail", () => {
    ...
    expect(() => {
      new MultisigContext(contextParams);
    }).toThrow();
  });
  ...
  it("it should use a context to initialize the MultisigContext", () => {
    const context = new Context(contextParams);
    const simpleStorageContext = new MultisigContext({
      myParam: "notDefault",
    }, context);
    ...
    simpleStorageContext.ipfs.map((ipfsClient) =>
      expect(ipfsClient).toBeInstanceOf(IpfsClient)
    );
